...
int linearSearch(const vector<int>& arr, int target) {
  ...
  'ternary-search': {
    java: `// Ternary Search in Java: O(log3 N) search on sorted arrays
public class TernarySearch {
    public static int ternarySearch(int[] arr, int target) {
        ...
        while (low <= high) {
            ...
            if (target < arr[mid1]) {
                high = mid1 - 1; // Search in left third [low..mid1-1]
            } else if (target > arr[mid2]) {
                low = mid2 + 1;  // Search in right third [mid2+1..high]
            } else {
                low = mid1 + 1;  // Search in middle third [mid1+1..mid2-1]
                high = mid2 - 1;
            }
        }
    ...
    python: `# Ternary Search in Python: O(log3 N) lookup
def ternary_search(arr, target):
    ...
    while low <= high:
        ...
        if target < arr[mid1]:
            ...
            high = mid2 - 1
    return -1`,
    cpp: `// Ternary Search in C++: O(log3 N) divide-and-conquer search
