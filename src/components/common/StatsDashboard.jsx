...
export default function StatsDashboard({ stats = {}, isRunning = false, isFinished = false, statusText = '' }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
      ...
      <div className="glass-card rounded-2xl p-3.5 border border-white/10 flex flex-col justify-between">
      ...
      {statEntries.slice(0, 4).map((stat) => (
        <div key={stat.label} className="glass-card rounded-2xl p-3.5 border border-white/10 flex flex-col justify-between">
          <span className="text-zinc-400 text-xs font-mono">{stat.label}</span>
          <span className="text-xl font-display font-bold text-white tracking-tight mt-1">
            {stat.value}
          </span>
        </div>
      ))}
