    ...
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-2xl p-5 border border-white/10 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <span className="text-xs font-mono text-zinc-400 uppercase tracking-widest">Scheduling Data Structures</span>
            <h2 className="text-xl font-display font-black text-white">Priority Queue (Scheduler Engine)</h2>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2">
          <div className="flex items-center gap-2 bg-zinc-900 p-1.5 rounded-xl border border-white/10">
            <span className="text-xs font-mono text-zinc-400 pl-2">Task:</span>
            <input
              type="text"
              value={taskName}
              onChange={(e) => setTaskName(e.target.value)}
              className="w-28 bg-black border border-white/15 rounded-lg px-2 py-1 text-xs font-mono text-white focus:outline-none focus:border-white"
            />
            <span className="text-xs font-mono text-zinc-400 pl-1">Priority:</span>
            <input
              type="number"
              value={priorityVal}
              onChange={(e) => setPriorityVal(e.target.value)}
              className="w-14 bg-black border border-white/15 rounded-lg px-2 py-1 text-xs font-mono text-white text-center focus:outline-none focus:border-white"
            />
            <button
              onClick={handleEnqueue}
              disabled={pq.length >= 8}
              className="flex items-center gap-1 px-3.5 py-1.5 rounded-lg bg-white text-black font-display font-bold text-xs hover:bg-zinc-200 transition-all disabled:opacity-40"
            >
          ...
          >
            ...
            <span>Serve Next (Top Priority)</span>
          </button>

          <button
            onClick={handleRandomFill}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-zinc-900 text-zinc-300 hover:text-white border border-white/10 hover:border-white/30 text-xs font-mono transition-all"
          >
      ...
      <div className="glass-card rounded-2xl p-6 border border-white/10 space-y-4 min-h-[340px] flex flex-col justify-center">
        <div className="flex items-center justify-between font-mono text-xs text-zinc-400">
          <span className="text-emerald-400 font-bold uppercase tracking-wider">▲ NEXT TO SERVE (Index 0)</span>
          <span className="text-zinc-500">LOWEST PRIORITY IN BUFFER ▼</span>
        </div>
          ...
          {pq.length === 0 ? (
            ...
            pq.map((task, idx) => {
              ...
              return (
                <div
                  key={task.id}
                  className={`p-4 rounded-2xl border transition-all duration-300 flex flex-col justify-between h-28 relative ${
                    isHighlight
                ...
                >
                  ...
                  <div>
                    <h4 className="text-sm font-display font-bold truncate">{task.name}</h4>
                  </div>

                  {isHead && (
                    <span className="absolute -top-2.5 right-4 text-[8px] uppercase tracking-wider px-2 py-0.5 bg-emerald-500 text-black font-extrabold rounded-md shadow-sm">
                      TOP PRIORITY
                    </span>
                  )}
                </div>
